...
def test_bootstrap_tworzy_admina_ustawia_nazwe_i_loguje(client, db):
    r = client.post("/api/onboarding/bootstrap",
                    json={"login": "pierwszyadmin", "haslo": "Mocn3-Haslo!", "nazwa_lokalu": "Moja Knajpa"})
    assert r.status_code == 201, r.text
    assert "access_token" in r.json()
    assert r.json()["user"]["rola"] == "admin"
    # Admin w bazie.
    assert db.query(models.User).filter_by(login="pierwszyadmin").count() == 1
    # Logowanie tym kontem działa.
    assert client.post("/api/auth/login",
                       json={"login": "pierwszyadmin", "haslo": "Mocn3-Haslo!"}).status_code == 200
...
def test_drugi_bootstrap_odrzucony_409(client):
    assert client.post("/api/onboarding/bootstrap",
                       json={"login": "adminpierwszy", "haslo": "Mocn3-Haslo!"}).status_code == 201
    # Instancja ma już administratora — brak przejęcia.
    r = client.post("/api/onboarding/bootstrap", json={"login": "ad663drugi", "haslo": "Mocn3-Haslo!"})
    assert r.status_code == 409


def test_bootstrap_waliduje_dane(client):
    assert client.post("/api/onboarding/bootstrap",
                       json={"login": "abc", "haslo": "Mocn3-Haslo!"}).status_code == 400   # login za krótki
    assert client.post("/api/onboarding/bootstrap",
                       json={"login": "adminvalid", "haslo": "slabe"}).status_code == 400    # hasło za słabe
