...
describe('reservationRoute', () => {
  it('rozpoznaje i odczytuje bezpośredni link do widoku rezerwacji', () => {
    const hash = '#/rezerwacje/kalendarz?data=2026-08-17&zakres=day&status=potwierdzona&rezerwacja=81'
  ...
  it('buduje kanoniczny link dla każdego kontekstu roboczego', () => {
    expect(buildReservationHash({
      ...
      '#/rezerwacje/baza?od=2026-07-01&do=2026-09-30&status=no_show&sort=nazwisko_asc&offset=50&rezerwacja=123',
    )
  ...
  it('nigdy nie zapisuje danych osobowych ani frazy wyszukiwania w URL', () => {
    ...
    expect(hash).not.toMatch(/(?:query|q|nazwisko|phone|telefon)=/)
  })
