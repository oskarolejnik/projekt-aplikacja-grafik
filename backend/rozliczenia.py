...
Wejście (kwoty w zł):
  ...
  • L         — gotówka NIEZAFISKALIZOWANA (zamykający lokal),
...
Reguły (potwierdzone na realnym przykładzie z arkusza):
  ...
  KASY       = Σ kasy + L + FV − IMP_kasy − zadatek ;  różnica = SUMA SZEF − KASY,
...
def policz_dzien(kelnerzy=None, fv=0.0, terminale=None, kasy=None, L=0.0,
                 zadatek=0.0, kp=0.0, kw=0.0, imp=None):
    kelnerzy = kelnerzy or []
    terminale = terminale or []
    kasy = kasy or []
    imp = imp or {}
    fv = float(fv or 0); L = float(L or 0)
    ...
    return {
        ...
        # podsumowanie końcowe: G do oddania (gotówka „dla szefa" minus L), T, SALA = G+T
        "G_do_oddania": round(suma_szef_G - L, 2),
        "T": suma_szef_T,
        "SALA": suma_szef,
    }
